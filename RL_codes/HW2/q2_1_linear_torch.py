...
class Linear(DQN):
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        q_best = rewards + gamma * torch.max(target_q_values, dim=1)[0] * ~done_mask
        q_now = torch.sum(q_values * F.one_hot(actions.long(),num_actions), dim=1)
        self.loss = F.mse_loss(q_best, q_now, reduction='mean')
        return self.loss
        ##############################################################
        ######################## END YOUR CODE #######################
...
if __name__ == '__main__':
    ...
    toc = time.time() # new
    print(f"Nips dqn Training time: {toc-tic}") # new
